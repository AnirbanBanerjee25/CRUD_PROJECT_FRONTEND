...
function EditStudent() {
    ...
    useEffect(() => {
        Axios.get(`https://crud-backend-0t5z.onrender.com/studentRoute/update-student/${id}`)
            .then((res) => {
                if (res.status === 200) {
    ...
    const handleSubmit = () => {
        ...
        Axios.put(`https://crud-backend-0t5z.onrender.com/studentRoute/update-student/${id}`, data)
            .then((res) => {
                if (res.status === 200) {
    ...
    return (
        <form onSubmit={handleSubmit}>
            {/* Pass the existing data to the StudentForm component */}
            <StudentForm
                getState={getState}
